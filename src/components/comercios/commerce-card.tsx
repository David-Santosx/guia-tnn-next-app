...
export default function CommerceCard({ commerce, isOpen }: CommerceCardProps) {
  const { id, name, description, phone, rate, hours, imageUrl, location } = commerce;
  ...
  const renderStars = (rating: number) => {
    ...
    for (let i = 0; i < fullStars; i++) {
      stars.push(
        <Star key={`full-${i}`} className="w-4 h-4 fill-brand-orange text-brand-orange" />
      );
    }
